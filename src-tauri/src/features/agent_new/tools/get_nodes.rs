...
//!
//! ```no_run
//! use crate::features::agent_new::tools::get_nodes::GetNodesListTool;
//!
...
impl GetNodesListTool {
    ...
    pub fn execute(&self, args: &GetNodesListArgs) -> Result<Vec<NodeInfo>, NodesToolError> {
        ...
        let conn = Connection::open(&db_path).map_err(|e| {
            NodesToolError(format!("Failed to open database: {}", e))
        })?;
...
mod tests {
    ...
    fn test_node_info_serialization() {
        ...
        assert!(json.contains("\"hasContent\":true"));
    }

    #[test]
    fn test_node_position() {
        let pos = NodePosition { x: 100.0, y: 200.0 };
    ...
    fn test_is_empty_value() {
        ...
        assert!(!is_empty_value(&serde_json::json!(123)));
    ...
    fn test_get_content_preview() {
        ...
        assert_eq!(get_content_preview(&serde_json::json!([])), None);
        assert_eq!(get_content_preview(&serde_json::json!({})), None);
        assert_eq!(get_content_preview(&serde_json::json!("")), None);
    }

    #[test]
    fn test_get_content_preview_long_string() {
        let long_string = "a".repeat(100);
        let preview = get_content_preview(&serde_json::json!(long_string));
        assert_eq!(preview, Some("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa...".to_string()));
    }

    #[test]
    fn test_get_content_preview_many_fields() {
        let many_fields = serde_json::json!({
            "a": 1, "b": 2, "c": 3, "d": 4, "e": 5
        });
        let preview = get_content_preview(&many_fields);
        assert_eq!(preview, Some("5 fields: a, b, c...".to_string()));
    }
    ...
    async fn test_tool_definition() {
        ...
        assert!(def.description.contains("node_type"));
        assert!(def.description.contains("is_empty"));
    }
    ...
    #[test]
    fn test_nodes_tool_error_display() {
        let err = NodesToolError("test error".to_string());
        assert_eq!(err.to_string(), "Nodes tool error: test error");
    }

    #[test]
    fn test_get_nodes_list_tool_clone() {
        let tool = GetNodesListTool::new("/test/path");
        let tool_clone = tool.clone();
        assert_eq!(tool.project_path(), tool_clone.project_path());
    }
}
