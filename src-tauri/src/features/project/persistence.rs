...
//!
//! SQLite-based project I/O operations for save/load/init.
...
pub fn init_project_sqlite(project_root: &Path, name: &str) -> Result<SynniaProject, AppError> {
    ...
    let conn = database::init_db(&db_path)
        .map_err(|e| AppError::Database(format!("Failed to init database: {}", e)))?;
    ...
    ).map_err(|e| AppError::Database(format!("Failed to insert project meta: {}", e)))?;
...
pub fn save_asset(project_root: &Path, asset: &Asset) -> Result<bool, AppError> {
    let db_path = get_db_path(project_root);
    let conn = database::open_db(&db_path)
        .map_err(|e| AppError::Database(format!("Failed to open database: {}", e)))?;
    ...
    let old_hash = history::persistence::get_current_hash(&conn, &asset.id)
    ...
    if hash_changed {
        if let Some(old) = old_hash {
            ...
            if let Some(old_value) = old_value {
                history::persistence::create_snapshot_if_changed(&conn, &asset.id, &old, &old_value)
...
fn save_project_meta(conn: &Connection, meta: &ProjectMeta) -> Result<(), AppError> {
    ...
    ).map_err(|e| AppError::Database(format!("Failed to save project meta: {}", e)))?;
...
fn load_viewport(conn: &Connection) -> Result<Viewport, AppError> {
    ...
    ).map_err(|e| AppError::Database(format!("Failed to load viewport: {}", e)))
}
...
fn save_viewport(conn: &Connection, viewport: &Viewport) -> Result<(), AppError> {
    ...
    ).map_err(|e| AppError::Database(format!("Failed to save viewport: {}", e)))?;
...
fn load_nodes(conn: &Connection) -> Result<Vec<SynniaNode>, AppError> {
    ...
    let nodes = stmt.query_map([], |row| {
        ...
        let style = style_json.and_then(|s| serde_json::from_str(&s).ok());
    ...
    nodes.collect::<Result<Vec<_>, _>>()
        .map_err(|e| AppError::Database(format!("Failed to load nodes: {}", e)))
}

fn save_nodes(conn: &Connection, nodes: &[SynniaNode]) -> Result<(), AppError> {
    conn.execute("DELETE FROM nodes", [])
        .map_err(|e| AppError::Database(format!("Failed to clear nodes: {}", e)))?;
    ...
    for node in nodes {
        ...
        ).map_err(|e| AppError::Database(format!("Failed to insert node: {}", e)))?;
    }
...
fn load_edges(conn: &Connection) -> Result<Vec<SynniaEdge>, AppError> {
    ...
    }).map_err(|e| AppError::Database(format!("Failed to query edges: {}", e)))?;
    
    edges.collect::<Result<Vec<_>, _>>()
        .map_err(|e| AppError::Database(format!("Failed to load edges: {}", e)))
}

fn save_edges(conn: &Connection, edges: &[SynniaEdge]) -> Result<(), AppError> {
    conn.execute("DELETE FROM edges", [])
        .map_err(|e| AppError::Database(format!("Failed to clear edges: {}", e)))?;
    ...
    for edge in edges {
        ...
        ).map_err(|e| AppError::Database(format!("Failed to insert edge: {}", e)))?;
    }
...
fn load_assets(conn: &Connection) -> Result<HashMap<String, Asset>, AppError> {
    ...
    for asset_result in rows {
        let asset = asset_result.map_err(|e| AppError::Database(format!("Failed to load asset: {}", e)))?;
...
fn load_settings(conn: &Connection) -> Result<Option<HashMap<String, serde_json::Value>>, AppError> {
    ...
    for row_result in rows {
        let (key, value) = row_result.map_err(|e| AppError::Database(format!("Failed to load setting: {}", e)))?;
...
fn save_settings(conn: &Connection, settings: &Option<HashMap<String, serde_json::Value>>) -> Result<(), AppError> {
    conn.execute("DELETE FROM settings", [])
        .map_err(|e| AppError::Database(format!("Failed to clear settings: {}", e)))?;
