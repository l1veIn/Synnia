...
// Must be imported first to set up the mock before importing AssetSystem
...
const createMockSys = (overrides: Partial<AssetSysMetadata> = {}): AssetSysMetadata => ({
    name: 'Test Asset',
    createdAt: Date.now(),
    updatedAt: Date.now(),
    source: 'user',
...
describe('AssetSystem', () => {
    ...
    let mockEngine: jest.Mocked<GraphEngine>;
    ...
    beforeEach(() => {
        ...
        mockEngine = {
            assets: assetSystem,
        } as unknown as jest.Mocked<GraphEngine>;
    ...
    describe('update', () => {
        ...
        it('should preserve other asset properties when updating value', () => {
            ...
            expect(updatedAsset?.config).toEqual({ schema: [{ key: 'test', type: 'string' }] });
    ...
    describe('updateConfig', () => {
        ...
        it('should merge config updates with existing config (shallow merge)', () => {
            ...
            expect(updatedAsset?.config.schema).toEqual([{ key: 'existing', type: 'string' }]);
    ...
    describe('delete', () => {
        ...
        it('should handle deleting non-existent asset', async () => {
            ...
            // Should not throw
            await expect(assetSystem.delete('non-existent-id')).resolves.toBeUndefined();
    ...
    describe('cleanupOrphans', () => {
        ...
        it('should return result from apiClient', async () => {
            ...
            vi.mocked(apiClient.cleanupOrphanAssets).mockResolvedValueOnce(expectedResult);
        ...
        it('should re-throw errors from apiClient', async () => {
            ...
            await expect(assetSystem.cleanupOrphans()).rejects.toThrow('Network error');
        });
    });
    ...
    describe('integration scenarios', () => {
        it('should handle complete asset lifecycle: create, update, updateConfig, updateSys, delete', async () => {
            // Create
            const id = assetSystem.create('record', { initial: 'value' }, { name: 'Test Asset' });
            ...
            // Update config
            assetSystem.updateConfig(id, { schema: [{ key: 'test', type: 'string' }] });
            asset = assetSystem.get(id);
        ...
        it('should manage multiple assets independently', () => {
            ...
            const asset1 = createMockAsset({ id: 'asset-1', value: { name: 'Asset 1' } });
