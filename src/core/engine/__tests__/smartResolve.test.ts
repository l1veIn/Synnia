...
import type { FieldDefinition, FieldType } from '@/types/assets';
...
describe('isRequiredSatisfied', () => {
    ...
    describe('required object fields with nested schema', () => {
        ...
        it('should return false when required sub-field is null', () => {
            expect(isRequiredSatisfied({ name: null, email: 'test@test.com' }, field)).toBe(false);
        });

        it('should return false when required sub-field is undefined', () => {
            expect(isRequiredSatisfied({ name: 'John', email: undefined }, field)).toBe(false);
        });

        it('should return true when all required sub-fields are satisfied', () => {
            expect(isRequiredSatisfied({ name: 'John', email: 'test@test.com' }, field)).toBe(true);
        });

        it('should return true when all required and optional sub-fields are present', () => {
            expect(isRequiredSatisfied({ name: 'John', age: 25, email: 'test@test.com' }, field)).toBe(true);
        });

        it('should return false for empty string in required string sub-field', () => {
            expect(isRequiredSatisfied({ name: '', email: 'test@test.com' }, field)).toBe(false);
        });
        ...
        it('should handle nested object validation recursively', () => {
            ...
            expect(isRequiredSatisfied({ address: { street: '123 Main', city: 'NYC' } }, nestedField)).toBe(true);
            expect(isRequiredSatisfied({ address: { street: '123 Main' } }, nestedField)).toBe(false);
        });
    });

    describe('unknown field types', () => {
        it('should return true for unknown type with value', () => {
            const field = createField({ type: 'unknown' as FieldType, required: true });
...
describe('smartResolve', () => {
    ...
    describe('keyed extraction strategy', () => {
        const field = createField({ key: 'username', type: 'string', required: true });
        ...
        it('should fall through to structural when key exists but invalid', () => {
            const objectField = createField({
                ...
                required: true,
                schema: [
                    { key: 'requiredField', type: 'string', required: true },
                ],
            });
        ...
        it('should include all missing required fields in error message', () => {
            const field = createField({
                ...
                schema: [
                    ...
                    { key: 'email', type: 'string', required: true, label: 'Email Address' },
    ...
    describe('rejection strategy (none)', () => {
        it('should return failure when no strategy matches for string type', () => {
            const field = createField({ key: 'missing', type: 'string', required: true });
        ...
        it('should return failure when no strategy matches for number type', () => {
            const field = createField({ key: 'count', type: 'number', required: false });
            const source = { other: 'value' };
            const result = smartResolve(source, field);
        ...
        it('should return failure when no strategy matches for boolean type', () => {
            const field = createField({ key: 'enabled', type: 'boolean', required: false });
            const source = { other: 'value' };
            const result = smartResolve(source, field);
        ...
        it('should return failure when no strategy matches for array type', () => {
            const field = createField({ key: 'items', type: 'array', required: false });
            const source = { other: 'value' };
            const result = smartResolve(source, field);
    ...
    describe('edge cases', () => {
        ...
        it('should handle non-required field with missing key', () => {
            const field = createField({ key: 'optional', type: 'string', required: false });
            const source = { other: 'value' };
            const result = smartResolve(source, field);
        ...
        it('should handle object type with no schema', () => {
            const field = createField({ key: 'data', type: 'object', required: true });
...
describe('smartResolveValue', () => {
    ...
    it('should return extracted value for direct match', () => {
        const field = createField({ key: 'status', type: 'boolean', required: false });
        const source = true;
...
describe('smartResolve integration scenarios', () => {
    describe('real-world form data extraction', () => {
        it('should resolve user profile data from form output', () => {
            ...
            const usernameField = createField({ key: 'username', type: 'string', required: true });
            const emailField = createField({ key: 'email', type: 'string', required: true });
            const ageField = createField({ key: 'age', type: 'number', required: false });
            const prefsField = createField({ key: 'preferences', type: 'object', required: false });

            expect(smartResolveValue(formOutput, usernameField)).toBe('johndoe');
            expect(smartResolveValue(formOutput, emailField)).toBe('john@example.com');
            expect(smartResolveValue(formOutput, ageField)).toBe(30);
            expect(smartResolveValue(formOutput, prefsField)).toEqual({ theme: 'dark' });
        });
        ...
        it('should validate required fields in complex form', () => {
            ...
            expect(smartResolveError(invalidUser, userField)).toContain('Missing required fields');
        });
    });
    ...
    describe('API response handling', () => {
        it('should handle array response from API', () => {
            ...
            const itemsField = createField({ key: 'items', type: 'array', required: true });
