    ...
    describe('checkSchemaCompatibility', () => {
        ...
        describe('when schemas are identical', () => {
            it('should return compatible with no warnings or changes', () => {
                const result = checkSchemaCompatibility(mockSnapshot, mockCurrentSchema);
            ...
            it('should handle schemas with same fields but different labels', () => {
                ...
                const result = checkSchemaCompatibility(mockSnapshot, currentWithDifferentLabels);
        ...
        describe('when fields are added to current schema', () => {
            it('should return compatible with added fields listed', () => {
                const result = checkSchemaCompatibility(mockSnapshot, extendedCurrentSchema);
            ...
            it('should detect multiple added fields', () => {
                ...
                const result = checkSchemaCompatibility(mockSnapshot, currentWithManyAdditions);
        ...
        describe('when fields are removed from current schema', () => {
            it('should return incompatible with removed fields listed', () => {
                const result = checkSchemaCompatibility(mockSnapshot, reducedCurrentSchema);
            ...
            it('should generate warning messages for removed fields with labels', () => {
                ...
                expect(result.warnings[0]).toBe('Field "Email" was removed from recipe');
            });
            ...
            it('should detect multiple removed fields', () => {
                const minimalCurrentSchema: SchemaField[] = [{ key: 'id', label: 'ID' }];

                const result = checkSchemaCompatibility(mockSnapshot, minimalCurrentSchema);
        ...
        describe('when fields are both added and removed', () => {
            it('should report both added and removed fields', () => {
                ...
                const result = checkSchemaCompatibility(mockSnapshot, currentWithChanges);
            ...
            it('should generate warnings for each removed field', () => {
                ...
                expect(result.warnings).toHaveLength(2);
                expect(result.warnings).toContain('Field "Name" was removed from recipe');
                expect(result.warnings).toContain('Field "Email" was removed from recipe');
            });
        });
        ...
        describe('edge cases', () => {
            ...
            it('should handle single field snapshot removal', () => {
                const singleFieldSnapshot: SchemaField[] = [{ key: 'onlyField', label: 'Only' }];
                const result = checkSchemaCompatibility(singleFieldSnapshot, []);

                expect(result.compatible).toBe(false);
                expect(result.removed).toEqual(['onlyField']);
                expect(result.warnings).toEqual(['Field "Only" was removed from recipe']);
            });
